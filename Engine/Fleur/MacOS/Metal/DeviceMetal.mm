...
DeviceMetal::DeviceMetal()
{
    ...
    FL_CORE_ASSERT(_device, "Failed to create device.");

    _defaultLibrary = _device->newDefaultLibrary();
    FL_CORE_ASSERT(_defaultLibrary, "Failed to load default library");
}
...
std::unique_ptr<Shader> DeviceMetal::CreateShader(std::string_view shaderName, Shader::ShaderType shaderType)
{
    ...
    FL_CORE_ASSERT(shader, "Failed to create shader");
