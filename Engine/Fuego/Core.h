#pragma once
    ...
    #elif defined(FUEGO_PLATFORM_MACOS)
        #define FU_ASSERT(x, ...) { if(!(x)) { FU_ERROR("Assertion Failed {0}", __VA_ARGS__); __builtin_debugtrap(); } }
        #define FU_CORE_ASSERT(x, ...) { if(!(x)) { FU_CORE_ERROR("Assertion Failed {0}", __VA_ARGS__); __builtin_debugtrap(); } }
    #endif
#else
    #define FU_ASSERT(x, ...)
